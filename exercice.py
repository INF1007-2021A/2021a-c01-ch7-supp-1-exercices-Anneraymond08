...
def get_fibonacci_sequence(length, seq=[0, 1]):
    return (seq[0:length] if length <= 2 else
            get_fibonacci_sequence(length, seq + [seq[-1] + seq[-2]]) if len(
                seq) < length else
            seq)


def get_sorted_dict_by_decimals(unsorted_dict):
    return dict(sorted(unsorted_dict.items(), key=lambda x: x[1] % 1.0))
...
def build_recursive_sequence_generator(initial_values: list, recursive_def, keep_whole_seq=False):
...
if __name__ == "__main__":
    print([get_fibonacci_number(0), get_fibonacci_number(1),
    ...
    lucas = build_recursive_sequence_generator([2, 1], lambda seq: seq[-1] + seq[-2], True)
    print(f"Lucas : {[elem for elem in lucas(10)]}")
    perrin = build_recursive_sequence_generator([3, 0, 2], lambda seq: seq[-2] + seq[-3], True)
